...
static int hpjack_status_check(void *priv)
{
	...
	if (hp_status == data->options.hp_active_level) {
		...
		if (data->options.iphone_jack || !gpio_is_valid(data->options.mic_gpio)) {
			snd_soc_dapm_force_enable_pin(&data->card.dapm, data->board_info->micbias_name);
		}
		...
		if (data->options.iphone_jack || !gpio_is_valid(data->options.mic_gpio))
			snd_soc_dapm_disable_pin(&data->card.dapm, data->board_info->micbias_name);
...
static int imx_wm896x_late_probe(struct snd_soc_card *card)
{
	...
	case CPUTYPE_IMX6:
		...
		snd_soc_component_update_bits(codec_dai->component,
					      WM8960_ADDCTL4, 0x1, 0x0);
		/* Use MONO mixer */
		snd_soc_component_update_bits(codec_dai->component,
					      WM8960_ADDCTL1, 0x10, 0x10);
		break;
...
static int imx_wm8960_probe(struct platform_device *pdev)
{
	...
	if (board_info->cpu_type == CPUTYPE_IMX6) {
		...
		data->dai.codecs->dai_name = "wm8960";
	}
	else {
