...
static int imx_sp2_hpjack_status_check(void *priv)
{
	...
	int hp_status, ret=0;
	hp_status = gpiod_get_value(data->options.hp_gpio) ? 1 : 0;
...
static int imx_sp2_micjack_status_check(void *priv)
{
	...
	mic_status = gpiod_get_value(data->options.mic_gpio) ? 1 : 0;
	if (mic_status) {
		data->mic_connected = true;
...
static int imx_sp2_audio_probe(struct platform_device *pdev)
{
	...
	data->options.hp_gpio = devm_gpiod_get_optional(&pdev->dev, "hp-det-gpios", GPIOD_IN);
	data->options.mic_gpio = devm_gpiod_get_optional(&pdev->dev, "mic-det-gpios", GPIOD_IN);
