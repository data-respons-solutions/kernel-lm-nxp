...
struct brcmf_sdio {
	...
	bool fcstate;		/* Flow control */
	struct pktq txq;	/* Queue length used for flow-control */
	struct sk_buff *tx_ctl_skb;
	bool tx_ctl_pending;
	atomic_t tx_data_pending;
	struct completion tx_complete;
	u8 flowcontrol;	/* per prio flow control bitmask */
	u8 tx_seq;		/* Transmit sequence number (next) */
	u8 tx_max;		/* Maximum transmit sequence allowed */
	ktime_t credit_time;	/* Credit renewed time */
	struct task_struct *tx_task;
	wait_queue_head_t tx_wait;
	wait_queue_head_t tx_credits_wait;
	...
	atomic_t rx_triggered;
	atomic_t tx_triggered;
...
static u8 tx_credits(struct brcmf_sdio *bus) {
	u8 head, tail;
	head = READ_ONCE(bus->tx_max);
	tail = READ_ONCE(bus->tx_seq);
	return (u8)(head - tail);
}
...
static u32 brcmf_sdio_hostmail(struct brcmf_sdio *bus)
{
	...
	if (hmb_data & HMB_DATA_FC) {
		...
		brcmf_info("Legacy flow control %d\n", fcbits);
			      ...
			      enum brcmf_sdio_frmtype type)
{
	...
	if ((u8)(tx_seq_max - READ_ONCE(bus->tx_seq)) > 0x40) {
		brcmf_err("seq %d: max tx seq number error\n", rx_seq);
		tx_seq_max = READ_ONCE(bus->tx_seq) + 2;
	}

	if (tx_seq_max != READ_ONCE(bus->tx_max)) {
		bus->credit_time = ktime_get();
		WRITE_ONCE(bus->tx_max, tx_seq_max);
		wake_up(&bus->tx_credits_wait);
	}
...
__maybe_unused static int brcmf_sdio_dcmd_resp_wait(struct brcmf_sdio *bus, uint *condition,
				     bool *pending)
{
			    ...
			    uint chan)
{
	...
	if (ret == 0)
		WRITE_ONCE(bus->tx_seq, (READ_ONCE(bus->tx_seq) + pktq->qlen) % SDPCM_SEQ_WRAP);
	skb_queue_walk_safe(pktq, pkt_next, tmp) {
		__skb_unlink(pkt_next, pktq);
...
int16_t brcmf_sdio_sendfromq(struct brcmf_sdio *bus, int num_pkts)
{
	struct sk_buff *pkt;
	struct sk_buff_head pktq;
	...
	if (bus->txglom) {
		while (cnt < num_pkts) {
			...
			pkt_num =
				min_t(u32, bus->sdiodev->txglomsz, num_pkts - cnt);
	...
	if ((bus->sdiodev->state == BRCMF_SDIOD_DATA) &&
	    READ_ONCE(bus->txoff) && (pktq_len(&bus->txq) < TXLOW)) {
		WRITE_ONCE(bus->txoff, false);
...
static int brcmf_sdio_tx_ctrlframe(struct brcmf_sdio *bus, u8 *frame, u16 len)
{
	...
	hd_info.seq_num = READ_ONCE(bus->tx_seq);
	...
	do {
		...
		if (ret < 0)
			...
			WRITE_ONCE(bus->tx_seq, (bus->tx_seq + 1) % SDPCM_SEQ_WRAP);
...
static void brcmf_sdio_bus_stop(struct device *dev)
{
	...
	if (sdiodev->state != BRCMF_SDIOD_NOMEDIUM) {
		brcmf_sdio_claim_host(sdiodev->func1);
...
static inline void brcmf_sdio_enable_irq(struct brcmf_sdio *bus)
{
	if (bus->sdiodev->oob_irq_requested) {
		enable_irq(bus->sdiodev->settings->bus.sdio.oob_irq_nr);
		WRITE_ONCE(bus->irq_en, true);
	}
}

static u32 brcmf_sdio_intr_rstatus(struct brcmf_sdio *bus)
{
	struct brcmf_core *core = bus->sdio_core;
	u32 addr;
	unsigned long val;
...
static void brcmf_sdio_ready(struct brcmf_sdio *bus)
{
	int err;
...
static bool brcmf_sdio_dpc(struct brcmf_sdio *bus)
{
	u32 intstatus=0;
	bool pending_ops = false;
	brcmf_dbg(SDIO, "Enter\n");
	brcmf_sdio_claim_host(bus->sdiodev->func1);
	intstatus = brcmf_sdio_intr_rstatus(bus) | bus->intstatus;
	...
	if (intstatus & I_HMB_FC_CHANGE) {
		...
		WRITE_ONCE(bus->fcstate, intstatus & I_HMB_FC_STATE ? true : false);
		brcmf_info("Flow change to %d\n", bus->fcstate);
		if (!bus->fcstate)
			wake_up(&bus->tx_credits_wait);
	}

	/* Group do not care interrupts */
	#define BRCMF_DONT_CARE_IRQ_MASK (I_WR_OOSYNC | I_RD_OOSYNC | I_SBINT)
	/* Generally don't ask for these, can get CRC errors... */
	if (intstatus & BRCMF_DONT_CARE_IRQ_MASK) {
		brcmf_err("Dongle reports dont cares 0x%x\n", intstatus & BRCMF_DONT_CARE_IRQ_MASK);
		intstatus &= ~BRCMF_DONT_CARE_IRQ_MASK;
	}
	...
	if (intstatus & I_HMB_FRAME_IND || bus->rxpending) {
		if (!bus->rxskip) {
			brcmf_sdio_readframes(bus, BRCMF_RXBOUND);
			if (bus->rxpending)
				pending_ops = true;
		} else
			pending_ops = true;
...
#endif
	atomic_xchg(&bus->tx_data_pending, 1);
	wake_up_interruptible(&bus->tx_wait);
	return ret;
}
...
__maybe_unused static int brcmf_sdio_readconsole(struct brcmf_sdio *bus)
{
...
brcmf_sdio_bus_txctl(struct device *dev, unsigned char *msg, uint msglen)
{
	...
	smp_mb();
	if (READ_ONCE(bus->tx_ctl_pending))
		return -EBUSY;

	skb = alloc_skb(msglen, GFP_KERNEL);
	memcpy(skb->data, msg, msglen);
	skb->cb[2] = SDPCM_CONTROL_CHANNEL;
	reinit_completion(&bus->tx_complete);
	WRITE_ONCE(bus->tx_ctl_skb, skb);
	WRITE_ONCE(bus->tx_ctl_pending, 1);
	wake_up_interruptible(&bus->tx_wait);
	ret = wait_for_completion_interruptible_timeout(&bus->tx_complete, msecs_to_jiffies(2500));
	if (ret == 0) {
		brcmf_err("Timeout\n");
		ret = -ETIMEDOUT;
	}
...
static int brcmf_debugfs_sdio_count_read(struct seq_file *seq, void *data)
{
	...
	seq_printf(seq,
		   ...
		   READ_ONCE(sdiodev->bus->tx_max), READ_ONCE(sdiodev->bus->tx_seq),
		   ktime_to_ms(sdiodev->bus->credit_time),
		   sdiodev->bus->irq_en);
	return 0;
}
...
brcmf_sdio_bus_rxctl(struct device *dev, unsigned char *msg, uint msglen)
{
	...
	/* Wait until control frame is available */
	timeleft = wait_event_interruptible_timeout(bus->dcmd_resp_wait, bus->rxlen > 0, DCMD_RESP_TIMEOUT);
...
void brcmf_sdio_isr(struct brcmf_sdio *bus, bool in_isr)
{
	...
	bus->irq_en = false;
	bus->sdcnt.intrcount++;
	queue_work(bus->brcmf_wq, &bus->rx_work);
}

static void brcmf_sdio_rx_worker(struct work_struct *work)
{
	bool pending = true;
	struct brcmf_sdio *bus = container_of(work, struct brcmf_sdio, rx_work);
	while (pending) {
		pending = brcmf_sdio_dpc(bus);
		if (pending)
			schedule();
	}
}

static int brcmf_sdio_tx_worker(void *data)
{
	int ret;
	struct brcmf_sdio *bus = data;
	brcmf_dbg(INFO, "Started TX thread\n");
	while (!kthread_should_stop()) {
		ret = wait_event_interruptible(
			bus->tx_wait, atomic_read(&bus->tx_data_pending) ||
					    READ_ONCE(bus->tx_ctl_pending));
		if (ret < 0)
			continue;

		/* Control plane date? */
		if (READ_ONCE(bus->tx_ctl_pending)) {
			ret = brcmf_sdio_tx_ctrlframe(bus,
						      bus->tx_ctl_skb->data,
						      bus->tx_ctl_skb->len);
			WRITE_ONCE(bus->tx_ctl_pending, 0);
			kfree_skb(bus->tx_ctl_skb);
			complete(&bus->tx_complete);
		}

		/* Ship netdev packets */
		if (atomic_xchg(&bus->tx_data_pending, 0)) {
			int pkts, tx_qlen, credits;
			spin_lock_bh(&bus->txq_lock);
			tx_qlen = brcmu_pktq_mlen(&bus->txq, ~bus->flowcontrol);
			spin_unlock_bh(&bus->txq_lock);

			ret = wait_event_interruptible_timeout(
				bus->tx_credits_wait,
				tx_credits(bus) > 2 && !bus->fcstate,
				msecs_to_jiffies(1000));
			if (ret == 0)
				ret = -ETIMEDOUT;
			if (ret < 0) {
				brcmf_err("Error on tx credits wait %d\n", ret);
				continue;
			}
			pkts = min_t(unsigned, credits, tx_qlen);
					 ...
					 struct brcmf_fw_request *fwreq)
{
	...
	bus->sdcnt.tickcnt = 0;
	brcmf_sdio_claim_host(sdiod->func1);
...
struct brcmf_sdio *brcmf_sdio_probe(struct brcmf_sdio_dev *sdiodev)
{
	...
	wq = alloc_ordered_workqueue("brcmf_wq/%s",
			     WQ_MEM_RECLAIM | WQ_HIGHPRI,
			     dev_name(&sdiodev->func1->dev));
	if (!wq) {
		brcmf_err("insufficient memory to create workqueue\n");
		goto fail;
	}
	brcmf_sdiod_freezer_count(sdiodev);
	INIT_WORK(&bus->rx_work, brcmf_sdio_rx_worker);
	bus->brcmf_wq = wq;
	init_completion(&bus->tx_complete);
	spin_lock_init(&bus->rxctl_lock);
	spin_lock_init(&bus->txq_lock);
	init_waitqueue_head(&bus->dcmd_resp_wait);
	init_waitqueue_head(&bus->tx_wait);
	init_waitqueue_head(&bus->tx_credits_wait);

	/* Initialize DPC thread */
	atomic_set(&bus->rx_triggered, 0);

	bus->tx_task = kthread_run(brcmf_sdio_tx_worker, bus, "brcmf-tx-worker");
	if (IS_ERR(bus->tx_task)) {
		brcmf_err("Failed to create tx thread\n");
	}
